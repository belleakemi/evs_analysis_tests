...
tryCatch({
  ...
  equacao_size <- "glmmTMB(tamanho_mean_average ~ wave * grupo_analise_dep + (1 | subjectid), family = gaussian(), dispformula = grupo_analise_dep)"
  cat("Model Equation: ", equacao_size, "\n")
  modelo_size <- tryCatch({
    glmmTMB(tamanho_mean_average ~ wave * grupo_analise_dep + (1 | subjectid), 
  ...
  p_interacao_size <- df_anova_size["wave:grupo_analise_dep", "Pr(>Chisq)"]
  if (!is.na(p_interacao_size) && p_interacao_size < 0.05) {
    ...
    # Check Group
    if (anova_size_type2["grupo_analise_dep", "Pr(>Chisq)"] < 0.05) {
      emm_size_traj <- emmeans(modelo_size, specs = pairwise ~ grupo_analise_dep, adjust = "tukey")
  ...
  modelo_size_vif <- glmmTMB(tamanho_mean_average ~ wave + grupo_analise_dep + (1 | subjectid), 
                             data = nanosight_intersect_ev_pequena_long, 
                             family = gaussian(), 
  ...
  cat("VIF:\n"); print(vif_df_conc); write.csv(vif_df_conc, paste0("analysis_results_depressao_controleoriginal/",  "table_vif_concentration_",  ".csv"), row.names = FALSE); cat("\nICC:\nNot applicable (GLM without random effects).\n")
  ...
  png(paste0("analysis_results_depressao_controleoriginal/",  "plot_diagnostics_concentration_",  ".png"), width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_conc, main = "A) QQ Plot - Concentration", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_conc, main = "B) Residuals vs. Predicted - Concentration"); dev.off()
  print(tabela_diag_conc); write.csv(tabela_diag_conc, paste0("analysis_results_depressao_controleoriginal/",  "table_diagnostics_dharma_concentration_",  ".csv"), row.names = FALSE)
  
  # MODEL 3: EVs percentages
  cat("\n\n======================================================================\n"); cat("  ANALYSIS 3: PERCENTAGE OF SMALL EVs (p90_porcentagem)\n"); cat("======================================================================\n\n")
  equacao_perc <- "glmmTMB(percentage_prop ~ wave * grupo_analise_dep + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = beta_family(link = logit), dispformula = grupo_analise_dep"
  ...
  if (p_interacao_perc < 0.05) {
    ...
    emm_size_p1 <- emmeans(modelo_perc, specs = pairwise ~ grupo_analise_dep | wave, adjust = "tukey"); df_emm_conc_p1 <- as.data.frame(emm_size_p1$contrasts); df_emm_conc_p1$Significance <- ifelse(df_emm_conc_p1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_p1); write.csv(df_emm_conc_p1, paste0("analysis_results_depressao_controleoriginal/",  "table_contrasts_wave_perc_",  ".csv"), row.names = FALSE)
    emm_size_p2 <- emmeans(modelo_perc, specs = pairwise ~ wave | grupo_analise_dep, adjust = "tukey"); df_emm_conc_p2 <- as.data.frame(emm_size_p2$contrasts); df_emm_conc_p2$Significance <- ifelse(df_emm_conc_p2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_p2); write.csv(df_emm_conc_p2, paste0("analysis_results_depressao_controleoriginal/",  "table_contrasts_grupo_analise_dep_perc_",  ".csv"), row.names = FALSE)
  } else {
    ...
    if (df_anova_perc2["wave", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'wave' is significant. Performing contrasts.\n\n"); emm_perc_wave <- emmeans(modelo_perc, specs = pairwise ~ wave, adjust = "tukey"); df_emm_perc_wave <- as.data.frame(emm_perc_wave$contrasts); df_emm_perc_wave$Significance <- ifelse(df_emm_perc_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_wave); write.csv(df_emm_perc_wave, paste0("analysis_results_depressao_controleoriginal/",  "/table_contrasts_wave_percentage_",  ".csv"), row.names = FALSE) }
    if (df_anova_perc2["grupo_analise_dep", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'grupo_analise_dep' is significant. Performing contrasts.\n\n");emm_perc_traj <- emmeans(modelo_perc, specs = pairwise ~ grupo_analise_dep, adjust = "tukey"); df_emm_perc_traj <- as.data.frame(emm_perc_traj$contrasts); df_emm_perc_traj$Significance <- ifelse(df_emm_perc_traj$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_traj); write.csv(df_emm_perc_traj, paste0("analysis_results_depressao_controleoriginal/",  "/table_contrasts_trajectory_percentage_",  ".csv"), row.names = FALSE) }
  }
  ...
  cat("VIF:\n"); print(vif_df_perc); write.csv(vif_df_perc, paste0("analysis_results_depressao_controleoriginal/",  "table_vif_percentage_",  ".csv"), row.names = FALSE)
  cat("\nICC:\n"); print(icc_perc)
  ...
  png(paste0("analysis_results_depressao_controleoriginal/",  "plot_diagnostics_dharma_percentage_",  ".png"), width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_perc, main = paste0("A) QQ Plot - Percentage "), testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_perc, main = paste0("B) Residuals vs. Predicted - Percentage")); dev.off()
  print(tabela_diag_perc); write.csv(tabela_diag_perc, paste0("analysis_results_depressao_controleoriginal/",  "table_diagnostics_dharma_percentage_",  ".csv"), row.names = FALSE)
  # AIC/BIC
  cat("\n\n=========================================================\n"); cat("  SUMMARY TABLE: AIC/BIC OF FINAL MODELS\n"); cat("=========================================================\n\n")
  print(resultados_finais_aic_bic)
  write.csv(resultados_finais_aic_bic, paste0("analysis_results_depressao_controleoriginal/",  "table_summary_aic_bic_final_",  ".csv"), row.names = FALSE)
...
# Certifique-se de que a coluna de grupo existe (rodando o início do seu script anterior)
# Se precisar recriar rapidinho para o gráfico:
nanosight_intersect_ev_pequena_long$grupo_analise_dep <- NA
nanosight_intersect_ev_pequena_long$grupo_analise_dep[nanosight_intersect_ev_pequena_long$traj_dep == "incidente"] <- "Incidente"
nanosight_intersect_ev_pequena_long$grupo_analise_dep[nanosight_intersect_ev_pequena_long$traj_dep == "remitente"] <- "Remitente"
nanosight_intersect_ev_pequena_long$grupo_analise_dep[nanosight_intersect_ev_pequena_long$traj_dep == "persistente"] <- "Persistente"
...
# Filtrar NAs
dados_plot <- nanosight_intersect_ev_pequena_long[!is.na(nanosight_intersect_ev_pequena_long$grupo_analise_dep), ]

# Definir ordem dos fatores
dados_plot$grupo_analise_dep <- factor(dados_plot$grupo_analise_dep, levels = c("Control", "Incidente", "Remitente", "Persistente"))

# GRÁFICO 1: TAMANHO MÉDIO (Size)
p1 <- ggplot(dados_plot, aes(x = grupo_analise_dep, y = tamanho_mean_average, fill = wave)) +
  ...
  theme_minimal() +
  labs(title = "Dispersão do Tamanho (Size) por Grupo e Wave",
       subtitle = "Depressao",
       x = "Grupo",
       y = "Tamanho Médio (nm)") +
...
print(p1)
ggsave("analysis_results_depressao/boxplot_size_wave.png", plot = p1, width = 8, height = 6)

# GRÁFICO 2: CONCENTRAÇÃO
p2 <- ggplot(dados_plot, aes(x = grupo_analise_dep, y = concentracao_real, fill = wave)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2), alpha = 0.6) +
  theme_minimal() +
  labs(title = "Dispersão da Concentração por Grupo e Wave",
       x = "Grupo",
...
print(p2)
ggsave("analysis_results_depressao/boxplot_conc_wave.png", plot = p2, width = 8, height = 6)

# GRÁFICO 3: PORCENTAGEM EVs PEQUENAS
p2 <- ggplot(dados_plot, aes(x = grupo_analise_dep, y = EV_pequenas_porcentagem, fill = wave)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2), alpha = 0.6) +
  theme_minimal() +
  labs(title = "Dispersão da Porcentagem por Grupo e Wave",
       x = "Grupo",
       y = "Porcentagem de EVs pequenas (%)") +
  scale_fill_brewer(palette = "Set2")

print(p2)
ggsave("analysis_results_depressao/boxplot_perc_wave.png", plot = p2, width = 8, height = 6)