...
if (!dir.exists("analysis_results_traj_gena_somente_wave")) { dir.create("analysis_results_traj_gena_somente_wave") }
cat("Packages loaded and results directory is ready.\n\n")

# LOG
log_file_name <- paste0("analysis_results_traj_gena_somente_wave/log_analise_", Sys.Date(), ".txt")
...
tryCatch({
  ...
  modelo_conc_tentativa1 <- tryCatch({glmmTMB(concentracao_real ~ wave + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = nbinom2(link = "log"))}, warning = function(w) {cat("CONVERGENCE WARNING in Attempt 1:\n", conditionMessage(w), "\n"); return(NULL)}, error = function(e) {cat("CONVERGENCE ERROR in Attempt 1:\n", conditionMessage(e), "\n"); return(NULL)})
  ...
  cat("VIF:\n"); print(vif_df_conc); write.csv(vif_df_conc, "analysis_results_traj_gena_somente_wave/table_vif_concentration.csv", row.names = FALSE); cat("\nICC:\nNot applicable (GLM without random effects).\n")
  ...
  png("analysis_results_traj_gena_somente_wave/plot_diagnostics_concentration.png", width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_conc, main = "A) QQ Plot - Concentration", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_conc, main = "B) Residuals vs. Predicted - Concentration"); dev.off()
  print(tabela_diag_conc); write.csv(tabela_diag_conc, "analysis_results_traj_gena_somente_wave/table_diagnostics_dharma_concentration.csv", row.names = FALSE)
  
  # AIC/BIC
  cat("\n\n=========================================================\n"); cat("  SUMMARY TABLE: AIC/BIC OF FINAL MODELS\n"); cat("=========================================================\n\n")
  print(resultados_finais_aic_bic)
  write.csv(resultados_finais_aic_bic, "analysis_results_traj_gena_somente_wave/table_summary_aic_bic_final.csv", row.names = FALSE)
  
}, finally = {
  # LOG
  cat("\n\n### FIM DO LOG DE ANÁLISE ###\n")
  cat("Todos os resultados foram salvos no diretório 'analysis_results_traj_gena_somente_wave'.\n")
