...
# LOG
log_file_name <- paste0("analysis_results_traj_anyhk/log_analise_", Sys.Date(), ".txt")
...
tryCatch({
  ...
  tryCatch({
    ...
    nanosight_intersect_ev_pequena_long$traj_anyhk <- as.factor(nanosight_intersect_ev_pequena_long$traj_anyhk)
  ...
  modelo_size_gaussiano <- lmer(tamanho_mean_average ~ wave * traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long)
  modelo_size_gamma <- glmmTMB(tamanho_mean_average ~ wave * traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = Gamma(link = "log"))
  ...
  print(tabela_comp_size); write.csv(tabela_comp_size, "analysis_results_traj_anyhk/table_model_comparison_size.csv", row.names = FALSE)
  ...
  png("analysis_results_traj_anyhk/plot_diagnostics_size_GAMMA.png", width = 3000, height = 1000, res = 300);  par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_gamma, main = "A) QQ Plot - Size (Gamma)", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_gamma, main = "B) Residuals vs. Predicted - Size (Gamma)"); dev.off()
  cat("Diagnostics for Gamma Model:\n"); print(tabela_diag_gamma); write.csv(tabela_diag_gamma, "analysis_results_traj_anyhk/table_diagnostics_dharma_size_GAMMA.csv", row.names = FALSE)
  ...
  png("analysis_results_traj_anyhk/plot_diagnostics_size_GAUSSIAN.png", width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_gaussiano, main = "A) QQ Plot - Size (Gaussian)", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_gaussiano, main = "B) Residuals vs. Predicted - Size (Gaussian)"); dev.off()
  cat("\nDiagnostics for Gaussian Model:\n"); print(tabela_diag_gaussiano); write.csv(tabela_diag_gaussiano, "analysis_results_traj_anyhk/table_diagnostics_dharma_size_GAUSSIAN.csv", row.names = FALSE)
  ...
  anova_size <- anova(modelo_size, type = 3); df_anova_size <- as.data.frame(anova_size); df_anova_size$Significance <- ifelse(df_anova_size$`Pr(>F)` < alfa, "Significant", "Not Significant"); print(df_anova_size); write.csv(df_anova_size, "analysis_results_traj_anyhk/table_anova_size.csv")
  ...
  if (p_interacao_size < 0.05) {
    ...
    emm_size_p1 <- emmeans(modelo_size, specs = pairwise ~ traj_anyhk | wave, adjust = "tukey"); df_emm_size_p1 <- as.data.frame(emm_size_p1$contrasts); df_emm_size_p1$Significance <- ifelse(df_emm_size_p1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_p1); write.csv(df_emm_size_p1, "analysis_results_traj_anyhk/table_contrasts_perspective1_size.csv", row.names = FALSE)
    emm_size_p2 <- emmeans(modelo_size, specs = pairwise ~ wave | traj_anyhk, adjust = "tukey"); df_emm_size_p2 <- as.data.frame(emm_size_p2$contrasts); df_emm_size_p2$Significance <- ifelse(df_emm_size_p2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_p2); write.csv(df_emm_size_p2, "analysis_results_traj_anyhk/table_contrasts_perspective2_size.csv", row.names = FALSE)
  } else {
    ...
    if (anova_size_type2["wave", "Pr(>F)"] < 0.05) { emm_size_wave <- emmeans(modelo_size, specs = pairwise ~ wave, adjust = "tukey"); df_emm_size_wave <- as.data.frame(emm_size_wave$contrasts); df_emm_size_wave$Significance <- ifelse(df_emm_size_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_wave); write.csv(df_emm_size_wave, "analysis_results_traj_anyhk/table_contrasts_wave_size.csv", row.names = FALSE) }
    if (anova_size_type2["traj_anyhk", "Pr(>F)"] < 0.05) { emm_size_traj <- emmeans(modelo_size, specs = pairwise ~ traj_anyhk, adjust = "tukey"); df_emm_size_traj <- as.data.frame(emm_size_traj$contrasts); df_emm_size_traj$Significance <- ifelse(df_emm_size_traj$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_traj); write.csv(df_emm_size_traj, "analysis_results_traj_anyhk/table_contrasts_trajectory_size.csv", row.names = FALSE) }
  }
  cat("\n--- 1.5. Model Diagnostics (Size) ---\n")
  cat("Assessing VIF on a model without the interaction term to prevent artificial inflation:\n")
  modelo_size_vif <- lmer(tamanho_mean_average ~ wave + traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long)
  ...
  cat("VIF:\n"); print(vif_df_size); write.csv(vif_df_size, "analysis_results_traj_anyhk/table_vif_size.csv", row.names = FALSE)
  cat("\nICC:\n"); print(icc_size)
  ...
  modelo_conc_tentativa1 <- tryCatch({glmmTMB(concentracao_real ~ wave * traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = nbinom2(link = "log"))}, warning = function(w) {cat("CONVERGENCE WARNING in Attempt 1:\n", conditionMessage(w), "\n"); return(NULL)}, error = function(e) {cat("CONVERGENCE ERROR in Attempt 1:\n", conditionMessage(e), "\n"); return(NULL)})
  if (is.null(modelo_conc_tentativa1)) {cat("\nAttempt 2: Negative Binomial GLMM (main effects only)...\n"); modelo_conc_tentativa2 <- tryCatch({glmmTMB(concentracao_real ~ wave + traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = nbinom1(link = "log"))}, warning = function(w) {cat("CONVERGENCE WARNING in Attempt 2:\n", conditionMessage(w), "\n"); return(NULL)}, error = function(e) {cat("CONVERGENCE ERROR in Attempt 2:\n", conditionMessage(e), "\n"); return(NULL)})}
  if(is.null(modelo_conc_tentativa1) && is.null(modelo_conc_tentativa2)) {cat("\nBoth mixed models failed. Selecting final model: GLM with main effects only.\n"); modelo_conc <- glmmTMB(concentracao_real ~ wave + traj_anyhk, data = nanosight_intersect_ev_pequena_long, family = nbinom1(link = "log"))}
  ...
  cat("VIF:\n"); print(vif_df_conc); write.csv(vif_df_conc, "analysis_results_traj_anyhk/table_vif_concentration.csv", row.names = FALSE); cat("\nICC:\nNot applicable (GLM without random effects).\n")
  ...
  png("analysis_results_traj_anyhk/plot_diagnostics_concentration.png", width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_conc, main = "A) QQ Plot - Concentration", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_conc, main = "B) Residuals vs. Predicted - Concentration"); dev.off()
  print(tabela_diag_conc); write.csv(tabela_diag_conc, "analysis_results_traj_anyhk/table_diagnostics_dharma_concentration.csv", row.names = FALSE)
  ...
  modelo_perc <- glmmTMB(percentage_prop ~ wave * traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = beta_family(link = "logit"), dispformula = ~ traj_anyhk)
  ...
  anova_perc_type3 <- Anova(modelo_perc, type = "III"); df_anova_perc3 <- as.data.frame(anova_perc_type3); df_anova_perc3$Significance <- ifelse(df_anova_perc3$`Pr(>Chisq)` < alfa, "Significant", "Not Significant"); print(df_anova_perc3); write.csv(df_anova_perc3, "analysis_results_traj_anyhk/table_anova_percentage.csv")
  ...
  if (p_interacao_perc < 0.05) {
    ...
    emm_perc_p1 <- emmeans(modelo_perc, specs = pairwise ~ traj_anyhk | wave, adjust = "tukey"); df_emm_perc_p1 <- as.data.frame(emm_perc_p1$contrasts); df_emm_perc_p1$Significance <- ifelse(df_emm_perc_p1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_p1); write.csv(df_emm_perc_p1, "analysis_results_traj_anyhk/table_contrasts_perspective1_percentage.csv", row.names = FALSE)
    emm_perc_p2 <- emmeans(modelo_perc, specs = pairwise ~ wave | traj_anyhk, adjust = "tukey"); df_emm_perc_p2 <- as.data.frame(emm_perc_p2$contrasts); df_emm_perc_p2$Significance <- ifelse(df_emm_perc_p2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_p2); write.csv(df_emm_perc_p2, "analysis_results_traj_anyhk/table_contrasts_perspective2_percentage.csv", row.names = FALSE)
  } else {
    ...
    if (df_anova_perc2["wave", "Pr(>Chisq)"] < 0.05) { emm_perc_wave <- emmeans(modelo_perc, specs = pairwise ~ wave, adjust = "tukey"); df_emm_perc_wave <- as.data.frame(emm_perc_wave$contrasts); df_emm_perc_wave$Significance <- ifelse(df_emm_perc_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_wave); write.csv(df_emm_perc_wave, "analysis_results_traj_anyhk/table_contrasts_wave_percentage.csv", row.names = FALSE) }
    if (df_anova_perc2["traj_anyhk", "Pr(>Chisq)"] < 0.05) { emm_perc_traj <- emmeans(modelo_perc, specs = pairwise ~ traj_anyhk, adjust = "tukey"); df_emm_perc_traj <- as.data.frame(emm_perc_traj$contrasts); df_emm_perc_traj$Significance <- ifelse(df_emm_perc_traj$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_traj); write.csv(df_emm_perc_traj, "analysis_results_traj_anyhk/table_contrasts_trajectory_percentage.csv", row.names = FALSE) }
  }
  cat("\n--- 3.4. Model Diagnostics (Percentage) ---\n")
  cat("Assessing VIF on a model without the interaction term to prevent artificial inflation:\n")
  modelo_perc_vif <- glmmTMB(percentage_prop ~ wave + traj_anyhk + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = beta_family(link = "logit"))
  ...
  cat("VIF:\n"); print(vif_df_perc); write.csv(vif_df_perc, "analysis_results_traj_anyhk/table_vif_percentage.csv", row.names = FALSE)
  cat("\nICC:\n"); print(icc_perc)
  ...
  png("analysis_results_traj_anyhk/plot_diagnostics_percentage.png", width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_perc, main = "A) QQ Plot - Percentage", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_perc, main = "B) Residuals vs. Predicted - Percentage"); dev.off()
  print(tabela_diag_perc); write.csv(tabela_diag_perc, "analysis_results_traj_anyhk/table_diagnostics_dharma_percentage.csv", row.names = FALSE)
  
  # AIC/BIC
  cat("\n\n=========================================================\n"); cat("  SUMMARY TABLE: AIC/BIC OF FINAL MODELS\n"); cat("=========================================================\n\n")
  print(resultados_finais_aic_bic)
  write.csv(resultados_finais_aic_bic, "analysis_results_traj_anyhk/table_summary_aic_bic_final.csv", row.names = FALSE)
