...
if (!dir.exists("analysis_results_traj_gena_disp_traj_somente_wave")) { dir.create("analysis_results_traj_gena_disp_traj_somente_wave") }
cat("Packages loaded and results directory is ready.\n\n")

# LOG
log_file_name <- paste0("analysis_results_traj_gena_disp_traj_somente_wave/log_analise_", Sys.Date(), ".txt")
...
tryCatch({
  ...
  modelo_conc_tentativa1 <- tryCatch({glmmTMB(concentracao_real ~ wave + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = nbinom2(link = "log"), dispformula = "traj_gena")}, warning = function(w) {cat("CONVERGENCE WARNING in Attempt 1:\n", conditionMessage(w), "\n"); return(NULL)}, error = function(e) {cat("CONVERGENCE ERROR in Attempt 1:\n", conditionMessage(e), "\n"); return(NULL)})
  if (is.null(modelo_conc_tentativa1)) {cat("\nAttempt 2: Negative Binomial GLMM (main effects only)...\n"); modelo_conc_tentativa2 <- tryCatch({glmmTMB(concentracao_real ~ wave + (1 | subjectid), data = nanosight_intersect_ev_pequena_long, family = nbinom1(link = "log"))}, warning = function(w) {cat("CONVERGENCE WARNING in Attempt 2:\n", conditionMessage(w), "\n"); return(NULL)}, error = function(e) {cat("CONVERGENCE ERROR in Attempt 2:\n", conditionMessage(e), "\n"); return(NULL)})}
  if(is.null(modelo_conc_tentativa1) && is.null(modelo_conc_tentativa2)) {cat("\nBoth mixed models failed. Selecting final model: GLM with main effects only.\n"); modelo_conc <- glmmTMB(concentracao_real ~ wave, data = nanosight_intersect_ev_pequena_long, family = nbinom1(link = "log"))}
  ...
  anova_conc <- Anova(modelo_conc, type = "II"); df_anova_conc <- as.data.frame(anova_conc); df_anova_conc$Significance <- ifelse(df_anova_conc$`Pr(>Chisq)` < alfa, "Significant", "Not Significant"); print(df_anova_conc); write.csv(df_anova_conc, "analysis_results_traj_gena_disp_traj_somente_wave/table_anova_concentration.csv")
  cat("\n--- 2.3. Pairwise Comparisons for Main Effects (Concentration) ---\n")
  if (df_anova_conc["wave", "Pr(>Chisq)"] < 0.05) {cat("\nMain effect of 'wave' is significant. Performing contrasts.\n"); emm_conc_traj <- emmeans(modelo_conc, specs = pairwise ~ wave, adjust = "tukey"); df_emm_conc_traj <- as.data.frame(emm_conc_traj$contrasts); df_emm_conc_traj$Significance <- ifelse(df_emm_conc_traj$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_traj); write.csv(df_emm_conc_traj, "analysis_results_traj_gena_disp_traj_somente_wave/table_contrasts_trajectory_concentration.csv", row.names = FALSE)}
  ...
  cat("VIF:\n"); print(vif_df_conc); write.csv(vif_df_conc, "analysis_results_traj_gena_disp_traj_somente_wave/table_vif_concentration.csv", row.names = FALSE); cat("\nICC:\nNot applicable (GLM without random effects).\n")
  ...
  png("analysis_results_traj_gena_disp_traj_somente_wave/plot_diagnostics_concentration.png", width = 3000, height = 1000, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 1); plotQQunif(residuos_conc, main = "A) QQ Plot - Concentration", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_conc, main = "B) Residuals vs. Predicted - Concentration"); dev.off()
  print(tabela_diag_conc); write.csv(tabela_diag_conc, "analysis_results_traj_gena_disp_traj_somente_wave/table_diagnostics_dharma_concentration.csv", row.names = FALSE)
  
  # AIC/BIC
  cat("\n\n=========================================================\n"); cat("  SUMMARY TABLE: AIC/BIC OF FINAL MODELS\n"); cat("=========================================================\n\n")
  print(resultados_finais_aic_bic)
  write.csv(resultados_finais_aic_bic, "analysis_results_traj_gena_disp_traj_somente_wave/table_summary_aic_bic_final.csv", row.names = FALSE)
  
}, finally = {
  # LOG
  cat("\n\n### FIM DO LOG DE ANÁLISE ###\n")
  cat("Todos os resultados foram salvos no diretório 'analysis_results_traj_gena_disp_traj_somente_wave'.\n")
